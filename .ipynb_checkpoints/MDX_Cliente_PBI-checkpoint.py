...
with Pyadomd(conn_str) as conn:
    with conn.cursor().execute(mdx) as cur:
        ...
        df_vol.drop(columns = ['[Clientes].[Cve Cliente].[Cve Cliente].[MEMBER_UNIQUE_NAME]',
                               '[Tiempo].[Cve Mes].[Cve Mes].[MEMBER_UNIQUE_NAME]',
                               '[Año].[Año Acomodo].[Año Acomodo].[MEMBER_UNIQUE_NAME]',
                               '[Tipo TPV].[Tipo PPV].[Tipo PPV].[MEMBER_UNIQUE_NAME]'], inplace=True)
        
        df_vol.columns = ['Cliente','Mes','Año','TPV','Ingreso Neto','CFC','CF','CU']
